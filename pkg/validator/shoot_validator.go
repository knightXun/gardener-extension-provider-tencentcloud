...
func (v *Shoot) validateShoot(ctx context.Context, shoot *core.Shoot, infraConfig *tencentcloud.InfrastructureConfig) error {
	// Network validation
	networkPath := field.NewPath("spec", "networking")

	if errList := tencentvalidation.ValidateNetworking(shoot.Spec.Networking, networkPath); len(errList) != 0 {
	...
	if errList := tencentvalidation.ValidateInfrastructureConfig(infraConfig, shoot.Spec.Networking.Nodes, shoot.Spec.Networking.Pods, shoot.Spec.Networking.Services); len(errList) != 0 {
		return errList.ToAggregate()
	}
	...
	// Shoot workers
	if errList := tencentvalidation.ValidateWorkers(shoot.Spec.Provider.Workers, infraConfig.Networks.Zones, fldPath); len(errList) != 0 {
		return errList.ToAggregate()
	}
...
func (v *Shoot) validateShootUpdate(ctx context.Context, oldShoot, shoot *core.Shoot) error {
	...
	if !reflect.DeepEqual(oldInfraConfig, infraConfig) {
		if errList := tencentvalidation.ValidateInfrastructureConfigUpdate(oldInfraConfig, infraConfig); len(errList) != 0 {
			return errList.ToAggregate()
		}
	}

	if errList := tencentvalidation.ValidateWorkersUpdate(oldShoot.Spec.Provider.Workers, shoot.Spec.Provider.Workers, fldPath.Child("workers")); len(errList) != 0 {
		return errList.ToAggregate()
	}
