provider "tencentcloud" {
  ...
  region = "{{ required "tencentcloud.region is required" .Values.tencentcloud.region }}"
}

// Import an existing public key to build a alicloud key pair
resource "tencentcloud_key_pairs" "publickey" {
...
resource "tencentcloud_vswitch" "vsw_z{{ $index }}" {
...
// specify EIP id
data "tencentcloud_eip" "eip_natgw_ds{{ $index }}" {
...
{{- if $createEip }}
// Create a new EIP.
resource "tencentcloud_eip" "eip_natgw_z{{ $index }}" {
...
resource "tencentcloud_eip_association" "eip_natgw_asso_z{{ $index }}" {
  eip_id = alicloud_eip.eip_natgw_z{{ $index }}.id
  instance_id   = {{ required "natGatewayID is required" $.Values.vpc.natGatewayID }}
}
